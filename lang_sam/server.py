...
class LangSAMAPI(ls.LitAPI):
    ...
    def predict(self, inputs: dict) -> dict:
        ...
        return {
            "results": {
                "masks": np.array(results["masks"]).tolist(),
                "boxes": np.array(results["boxes"]).tolist(),
                "scores": np.array(results["scores"]).tolist(),
                "labels": np.array(results["labels"]).tolist(),
            },
    ...
    def encode_response(self, output: dict) -> Response:
        ...
        Returns:
            Response: Contains the processed image and results in JSON format.
        """
        try:
            ...
            response_content = {
                "image": image_base64,
                "results": output["results"],  # Assuming results is a dictionary
            }
